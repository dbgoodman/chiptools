...
def run_bowtie(lib_prefix, prefix_fn, 
    ...
    if not bowtie_cmd:
        # bowtie_cmd = '''bowtie -v 5 -l 28 -k 5000 -p 16 \
        #             --norc --best --strata --suppress 2,6 \
        #             -f %(lib_prefix)s %(unique_fa)s'''
        bowtie_cmd = '''
...
def load_aligns(lib_fasta_fn, align_fasta_fn, type):
    ...
    rna_rec['seq_len'] = map(len,rna_rec['seq'])
    
    #add library length column
    rna_rec['lib_len'] = [len(lib_seqs[lib_id]) 
        for lib_id in rna_rec['lib_id']]
    
    #add righthand offset column
    rna_rec['r_offset'] = sum(
        [rna_rec['lib_len'], -rna_rec['seq_len'], -rna_rec['l_offset'] ])
    return rna_rec

lib_fasta_fn = "/scratch/dbg/ecre/fa/202.norestrict.fa"
...
# lib_fasta_fn = "/scratch/dbg/ecre/fa/202.norestrict.fa"
# rna_fasta_fn = "/scratch/dbg/ecre/ct/202_hsrna/202_hsrna.counts.fa"
# dna_fasta_fn = "/scratch/dbg/ecre/ct/202_hsdna/202_hsdna.counts.fa"
# prefix_fn = "/scratch/dbg/ecre/202_hs"
